...
public class GameViewLevel5 extends MainView implements Runnable {
    private Thread thread;

    private Eagle[] eagles;

    private Backgrounds backgrounds_level5;

    private GameActivityLevel5 game_activity;

    public GameViewLevel5(Context context) {
        super(context);
    }

    public GameViewLevel5(Context context, AttributeSet attrs) {
        super(context, attrs);
    }

    public GameViewLevel5(GameActivityLevel5 activity) {
        super(activity);

        game_activity = activity;

        backgrounds_level5 = new Backgrounds(activity, screen_width, screen_height, parameters.BACKGROUND_IMAGES_LEVEL5);

        init_eagles();

        init_reset(parameters.LEVEL5_STR);
    }
    ...
    private void init_reset(String level) {
        ...
        if(in_game) {
            ...
            get_backgrounds(backgrounds_level5, parameters.BACKGROUNDS_STR + level);
    ...
    private void draw() {
        ...
        if (getHolder().getSurface().isValid()) {
            ...
            // Draw everything but sun popup and backgrounds
            draw_all(canvas);
            ...
            if(num_lives > 0) {
                draw_flag_popup(canvas, backgrounds_level5, eagles, "level_5");
            }
            ...
            if(hit_bird && num_lives > 0) {
                ...
                String game_state_id = parameters.GAME_STATE_STR + parameters.LEVEL5_STR;

                save_game_state(game_state_id, true);

                num_lives--;

                String num_lives_id = parameters.NUM_LIVES_STR + parameters.LEVEL5_STR;
    ...
    private void restart_game() {
        ...
        try {
            Thread.sleep(2000);
            game_activity.startActivity(new Intent(game_activity, GameActivityLevel5.class));
    ...
    private void quit_game() {
        ...
        save_game_state(parameters.GAME_STATE_STR + parameters.LEVEL5_STR, false);

        num_lives = parameters.NUM_LIVES;

        save_num_lives(parameters.NUM_LIVES_STR + parameters.LEVEL5_STR);
    ...
    private void save_high_score() {
        ...
        if (prefs.getInt("high_score_level5", 0) < score) {
            SharedPreferences.Editor editor = prefs.edit();
            editor.putInt("high_score_level5", score);
            editor.apply();
        }
    }
            ...
            case MotionEvent.ACTION_DOWN:
                ...
                if(in_pause_area && !hit_bird) {
                    ...
                    if(local_x >= left_continue && local_x <= right_continue && local_y >= top_continue && local_y <= bottom_continue) {
                        draw_continue_pressed(left_continue, top_continue, left_restart, top_restart);
                        restart_game();
                    }
                    else if(local_x >= left_restart && local_x <= right_restart && local_y >= top_restart && local_y <= bottom_restart) {
                        draw_restart_pressed(left_continue, top_continue, left_restart, top_restart);
                        save_high_score();
                        quit_game();
                    }
                }
